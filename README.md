...
2、派生类可以继承父类中所有public和protected成员变量和成员函数。父类中的private的成员不能被继承，但可以通过父类中的public和protected成员访问。
3、基类指针可以指向派生类对象，但是不能通过基类的指针访问派生类成员；
派生类指针不能指向基类对象。
4、多继承，一个派生类可以继承多个基类。

十四、多态
程序可以有多个不同的类，且每个类可以都有一个名称相同但具有不同实现的函数。
1、虚函数：在父类中使用virtual关键字声明的函数 。在子类重新定义父类中定义的虚函数时（父类虚函数为virtual A，派生类为A），会告诉编译器不要静态链接到父类该函数（virtual A），用指针调用函数A时会直接调用子类的函数A。
2、纯虚函数：父类中定义virtual A=0。
1）父类中的纯虚函数只是对函数进行了声明，没有定义；
2）定义有纯虚函数的父类就是用来被继承，即规定每个继承的子类都要有这个函数；
3）父类中有纯虚函数，则这个类不能用来实例化对象（定义对象）；
3、C++接口（抽象类）
接口描述了类的行为和功能，是标准和规范，而不需要完成类的功能实现。拥有一个纯虚函数这个类就是抽象类。
1）抽象类的目的就是为了给其他类提供一个可以继承的父类；
2）抽象类不能被实例化，只能作为接口使用；
3）一个父类的子类想要被实例化（定义），必须要实现父类中的每一个虚函数（和纯虚函数），这意味着C++支持使用父类声明接口。如果子类没有重写（定义实现）纯虚函数，就尝试实例化该类对象，就会导致编译错误。
4）可以用于实例化对象的类称为具体类。
